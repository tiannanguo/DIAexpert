...
$in0=shift;  #../hye_6sw_X.txt
$first_file_name=shift;  #hye1_8R_hye_all_frag_X_addDecoy_part1.chrom.txt.gz

# modified by Wu Jianfeng
# we need output directory, since the current directory may not be the nfs data directory.
$output_dir = shift;
if (!$output_dir) {
  $output_dir = ".";
}
...
$first_file_name=~/^([^_]+)_.*_part(\d+)\.chrom/;
$first_sample_name=$1;          #print "haha ",$1,"\n";
$num_part=$2;

# modified by Wu Jianfeng
# get the right sample name in case $first_file_name has directory prefix
$first_sample_name =~ /^.*\/([^\/]+$)/;
$first_sample_name = $1;
...
# modified by Wu Jianfeng
#$o="com_chrom_$1\_part$num_part.txt";
$o = $output_dir."com_chrom_$1\_part$num_part.txt";

print "ha $first_file_name,$first_sample_name, $o\n";
...
foreach my $d(@d){
          my @dd=&s($d);
          # modified by Wu Jianfeng
          #push @sample_id, $dd[1];   #print "gaga $dd[1]\n";
          push @sample_id, trim($dd[1]);
}
...
foreach my $sample (@sample_id){               #eg, pcp1
        #ms2
        my $in=$first_file_name;    #"$sample\_$num_win.chrom.txt.gz";  print $in, "\n";
        $in=~s/$first_sample_name/$sample/;
        print "read file $in\n";

        my $z = new IO::Uncompress::Gunzip $in;
        # modified by Wu Jianfeng
        if (!$z) {
            next;
        }

        print "file handler $z\n";

        while(my $d=$z->getline()) {   #print $d;
                chomp $d;
                $d =~ s/\r//g;
                my @dd=&s($d);
                $chrom{$dd[3]}->{$dd[0]}->{$sample}->{rt}=$dd[1];
                $chrom{$dd[3]}->{$dd[0]}->{$sample}->{i}=$dd[2];
                $tg{info}->{$dd[3]}="$dd[4]\t$dd[5]\t$dd[6]\t$dd[7]\t$dd[8]\t$dd[9]\t$dd[10]\t$dd[11]";
                $tg{frag}->{$dd[3]}->{$dd[0]}=1;
        }
        $z->close();
...
#remove header
delete $chrom{transition_group_id};
delete $tg{info}->{transition_group_id};
delete $tg{frag}->{transition_name};

open(OUT,">$o");
print OUT "$title\n";

foreach my $tg (keys %{$tg{info}}){
        foreach my $frag (keys %{$tg{frag}->{$tg}}){
                print OUT "$frag\t$tg\t$tg{info}->{$tg}";

                foreach my $sample (@sample_id){  #eg, pcp1
                        print OUT "\t$chrom{$tg}->{$frag}->{$sample}->{rt}";
                        print OUT "\t$chrom{$tg}->{$frag}->{$sample}->{i}";
                }
                print OUT "\n";
        }
}
...
sub spearman_cor{
    my $r=shift;  #\@m
    my $r2=shift; #\@n
    my $c1 = Statistics::RankCorrelation->new($r, $r2);
    return $spearman1=$c1->spearman;
}
...
sub pearson_cor{
    ...
    $stat->add_data(@$r);
    my %hash = ();
    @hash{'q', 'm', 'r', 'err'} = $stat->least_squares_fit(@$r2);
    return $hash{r};
}
