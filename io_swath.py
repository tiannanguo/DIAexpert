...
def get_tg_list(chrom_file):
    ...
    input_file = gzip.open(chrom_file, 'rb') if chrom_file.endswith(".gz") else open(chrom_file, 'r')
...
def use_one_best_sample(chrom_file):
    ...
    # modified by Wu Jianfeng
    if chrom_file.endswith(".txt"):
        chrom_file2 = chrom_file.replace('.txt', '_2.txt.gz')
    else:
    ...
    # modified by Wu Jianfeng
    input_file = gzip.open(chrom_file, 'rb') if chrom_file.endswith(".gz") else open(chrom_file, 'r')
    with input_file as i, gzip.open(chrom_file2, 'wb') as o:
        r = csv.DictReader(i, delimiter="\t")
...
def read_tic_normalization_file(tic_normalization_file, tic_key):
    ...
    with open(tic_normalization_file, 'rb') as i:
        r = csv.DictReader(i, delimiter="\t")
        for row in r:
            norm_factor[row['sample']] = row[tic_key]
