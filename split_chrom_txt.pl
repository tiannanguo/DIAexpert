...
$in=shift; #hye1_8R_hye_all_frag_X_addDecoy.chrom.txt.gz
$in2=shift; #"../3R_hye12sw_ref_pg_X_refined.txt";
$num_parts=shift;


my $title;

#get tg list from ref_pg file
my %tg_list;
my @d2=&oF($in2);
foreach (@d2){
          my @dd=&s($_);
...
#divide tg list into portions
my @tg=keys %tg_list;
my $total_tg_num=scalar(@tg);
my %portion=&divide_large_number($total_tg_num, $num_parts);


# read chrom data
my %chrom; #info from chrom.txt.gz file
my $z = new IO::Uncompress::Gunzip $in;
while (my $d=$z->getline()){
       if ($d=~/transition_name/){
            $d =~ s/\r//g;
            $title=$d;
       }
       else{
...
#write data
foreach my $i (sort {$a<=>$b} keys %portion){

        my $n1=$portion{$i}->{low};
        my $n2=$portion{$i}->{high};
        print "portion $i, $n1, $n2\n";


        my $o=$in;
        $o=~s/\.chrom\.txt\.gz/\_part$i\.chrom\.txt/;
        open(OUT,">$o");
        print OUT $title;
        foreach my $n ($n1..$n2){
              my $tg=$tg[$n-1];
              if ($chrom{$tg}){
                   foreach my $frag(keys %{$chrom{$tg}}){
                           print OUT "$chrom{$tg}->{$frag}\n";
                   }
              }
        }
        close OUT;
